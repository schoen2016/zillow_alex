...
# df['links'] = df['links'].replace('<aclass="list-card-linklist-card-link-top-margin"href="', ' ', regex=True)
...
# df1['links'] = df1['links'].replace('<aclass="list-card-linklist-card-link-top-margin"href="', ' ', regex=True)
...
df['beds'] = df['beds'].replace('<abbr class="list-card-label"> <!-- -->sqft</abbr></li><li class="list-card-statusText">-<abbr class="list-card-label"> <!-- -->Active</abbr>', ' ', regex=True)
...
#     soup = BeautifulSoup(r.content, 'html.parser')
#     pprint(soup)
#     home_value = soup.select_one('h4:-soup-contains("Home value")')
    # print(home_value)
    # if not home_value:
        # print(soup.select_one('.zestimate'))
